...
int main(void){
  ...
  printf("\n\n");
  ////////////
  fgets(name, 100, circuito);
  for(i=0, ninp=0, nout=0, ngate=0;feof(circuito)==0;i++){
    fscanf(circuito, "%s", h);
    if(strcmp(h,"INPUT")==0){
      fscanf(circuito,"%d", &input[ninp].output);
      fscanf(circuito,"%s", input[ninp].letter);
      ninp++;
    }
    else if(strcmp(h,"OUTPUT")==0){
      fscanf(circuito,"%d", &output[nout].input);
      fscanf(circuito,"%s", output[nout].letter);
      nout++;printf("\nout i = %d nout = %d\n", i,nout);
    }
    else if(strcmp(h,"NOT")==0){
      fscanf(circuito, "%d", &gate[ngate].input[0]);
      gate[ngate].input[1]=-1;
      fscanf(circuito, "%d", &gate[ngate].output);
      gate[ngate].type = 3;
      ngate++;
    }
    else{
      fscanf(circuito, "%d", &gate[ngate].input[0]);
      fscanf(circuito, "%d", &gate[ngate].input[1]);
      fscanf(circuito, "%d", &gate[ngate].output);
      if(strcmp(h, "AND")==0){
        gate[ngate].type = 1;
      }
